...
export default function updateMirrorStackedAreaChart(
    ...
    const products = Array.from(d3.union(data.map((d) => d.product))).sort(
        d3.ascending,
    );
    ...
    const stack = d3
        .stack<Map<string, { value: number }>, string>()
        .keys(products)
        .value(([, D]: any, key) => D.get(key)?.value ?? 0);

    const importSeries = stack(
        Array.from(importIndex.entries()).map(
            ([date, D]) => [new Date(date), D] as any,
        ),
    );

    const exportSeries = stack(
        Array.from(exportIndex.entries()).map(
            ([date, D]) => [new Date(date), D] as any,
        ),
    );
    ...
    products.forEach((product) => {
        if (!colorRegistry.has(product)) {
            ...
            if (available) {
                //si on trouve une couleur disponible on l'affecte
    ...
    /* =========================
       TOOLTIP
    ========================= */
    let Tooltip: d3.Selection<any, any, any, any> = d3
        .select("body")
    ...
    function drawAreas(group: any, series: any, area: any, type: string) {
        group
            ...
            .on("mousemove", function (event: any, d: any) {
              ...
              ${t("type")} ${type}<br>
              ${t("value")} ${d3.format(",.0f")(value).replace(/,/g, " ")} k€<br>
              ${t("date")} ${d3.timeFormat("%Y-%m-%d")(closest.data[0])}
            `,
                    )
    ...
    drawAreas(gExport, exportSeries, areaExport, t("export"));
    drawAreas(gImport, importSeries, areaImport, t("import"));
    ...
    if (legend.empty()) {
        legend = root
            ...
            .attr(
                "transform",
                `translate(${width - marginRight + 10}, ${marginTop})`,
            );
    }
            ...
            (update) =>
                update.call((update) => {
                    ...
                    update
                        ...
                        .selectAll("tspan")
                        .data((d: any) =>
                            wrapText(d, LEGEND_MAX_CHARS_PER_LINE),
                        )
                        .enter()
                        .append("tspan")
                        .attr("x", 20)
   ...
   ICONS
    ...
    iconsGroup
        ...
        .join(
            ...
            (update) => {
                ...
                return update.transition().duration(500).style("opacity", 1);
            },
    ...
    if (zoomRect.empty()) {
        zoomRect = plotGroup
            .insert("rect", ":first-child") // derrière tous les éléments
